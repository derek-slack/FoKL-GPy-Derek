...
def gibbs(inputs, data, phis, Xin, discmtx, a, b, atau, btau, draws, phind, xsm, sigsqd, tausqd, dtd):
    ...
    X = X[:, 0:mmtx + 1]
    pass  # return betas, sigs, taus, betahat, X, ev 
...
def fit(self, inputs=None, data=None, sampler = 'gibbs', **kwargs):
    ...
    samplers = {'gibbs': gibbs, 'gibbs_update': gibbs_Xin_update}

    if sampler not in samplers:
        raise ValueError(f"Unknown sampler {samplers}. Available samplers are: {list(samplers.keys())}.")
